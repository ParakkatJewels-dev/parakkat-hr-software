...
const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'hrms-payroll-db-'));
...
try {
  ...
  const output = run('psql', [...connection, '-d', 'hr_role_matrix_audit', '-XqAt', '-f', path.join(__dirname, 'payroll-probes.sql')]);
  ...
  fs.writeFileSync(path.join(__dirname, 'payroll-results.json'), JSON.stringify(report, null, 2) + '\n');
