...
// Generated by the isolated database tests from all production migrations, so these checks
// exercise the grants users actually receive instead of a second hand-maintained role matrix.
const ROLE_GRANTS = JSON.parse(readFileSync(new URL('../test/standardRolePermissions.json', import.meta.url), 'utf8'));
const SELF_GRANTS = ROLE_GRANTS.employee.permissions;
const SCOPED_GRANTS = Object.fromEntries(Object.entries(ROLE_GRANTS).map(([role, data]) => [role, data.permissions]));
...
test('Administration and its direct routes are limited to admins and HR among standard roles', () => {
  const tabs = OVERSIGHT_NAV.find((section) => section.id === 'admin').tabs;
  for (const role of ['zonal_manager', 'branch_manager', 'dept_head', 'employee']) {
    const predicates = predicatesFor(permsFor(role));
    const sections = visibleSections(role, predicates);
    assert.ok(!sections.some((section) => section.id === 'admin'), `${role}: no Administration menu`);
    for (const tab of tabs) assert.equal(canSeeTab(tab, predicates), false, `${role}: /${tab.id}`);
    assert.ok(!mobilePrimarySections(sections, role).some((section) => tabs.some((tab) => tab.id === section.id)));
  }
  for (const role of ['entity_admin', 'hr_manager', 'super_admin']) {
    const predicates = predicatesFor(role === 'super_admin' ? [] : permsFor(role), { isSuperAdmin: role === 'super_admin' });
    const admin = visibleSections(role, predicates).find((section) => section.id === 'admin');
    assert.ok(admin, `${role}: Administration remains available`);
    for (const id of ['administration', 'admin-roles']) {
      assert.ok(admin.tabs.some((tab) => tab.id === id), `${role}: ${id}`);
    }
  }
});
