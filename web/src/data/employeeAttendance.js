...
export function summarise(rows) {
  ...
  // Punctuality by weekday — Monday-morning lateness is a real and visible pattern. Counted over
  // days attended, so a weekday spent absent does not read as a weekday spent on time.
  const byDow = Array.from({ length: 7 }, () => ({ total: 0, late: 0 }));
