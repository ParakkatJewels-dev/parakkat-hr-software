-- 0097_named_salary_gross_components.sql
-- Salary structures can now carry per-employee named gross components in notes:
--   {"gross_components":[{"name":"HRA","amount":5000},{"name":"Travel","amount":750}]}
--
-- Payroll still uses salary_structures.gross as the agreed monthly gross. These rows only break
-- that gross down into named earning lines on the payslip, prorated by paid days just like Basic.
-- Existing plain-text notes are ignored safely.

create or replace function app.salary_gross_components(_notes text)
returns table(component_name text, component_amount numeric, component_order int)
...
begin
  ...
  return query
  select
    btrim(part.item->>'name') as component_name,
    (part.item->>'amount')::numeric as component_amount,
    part.ord::int as component_order
  from jsonb_array_elements(
    case
      when jsonb_typeof(_payload->'gross_components') = 'array' then _payload->'gross_components'
      else '[]'::jsonb
    end
  ) with ordinality as part(item, ord)
  where nullif(btrim(coalesce(part.item->>'name', '')), '') is not null
    and coalesce(part.item->>'amount', '') ~ '^\s*\d+(\.\d+)?\s*$'
    and (part.item->>'amount')::numeric >= 0
  order by part.ord;
end $$;

grant execute on function app.salary_gross_components(text) to authenticated;
...
begin
  ...
  loop
    ...
    for _sal_comp in select * from app.salary_gross_components(_sal.notes) loop
      _sal_comp_paid := round((_sal_comp.component_amount / _days) * _paid, 2);
