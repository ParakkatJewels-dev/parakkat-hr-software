-- PAY-01..04: restore attendance-based unpaid days and fixed-component proration,
-- reconcile named/catalog earnings to agreed gross, and make publication readable.
-- This changes configured software arithmetic only; no statutory rates are set here.
-- Safe to reapply. Existing published amounts are never recomputed by this migration.
begin;

create or replace function public.run_payroll(_entity_id uuid, _period text)
returns jsonb
language plpgsql security definer set search_path = app, public as $$
declare
  _from          date;
  _to            date;
  _days          int;
  _run_id        uuid;
  _emp           record;
  _sal           record;
  _sal_comp      record;
  _comp          record;
  _lop           numeric(6,2);
  _paid          numeric(6,2);
  _gross_paid    numeric(12,2);
  _basic_paid    numeric(12,2);
  _sal_comp_paid numeric(12,2);
  _structure_total numeric;
  _earn          numeric(12,2);
  _named_earn    numeric(12,2);
  _other         numeric(12,2);
  _ded           numeric(12,2);
  _emp_cost      numeric(12,2);
  _base          numeric(12,2);
  _amt           numeric(12,2);
  _payslip_id    uuid;
  _count         int := 0;
  _sum_gross     numeric(14,2) := 0;
  _sum_net       numeric(14,2) := 0;
begin
  if _period !~ '^\d{4}-\d{2}$' then
    raise exception 'period must be YYYY-MM';
  end if;
  if auth.uid() is not null
     and not app.has_perm('payroll.manage', _entity_id, null, null, null, null) then
    raise exception 'not authorized to run payroll for this entity';
  end if;

  _from := (_period || '-01')::date;
  _to   := (_from + interval '1 month - 1 day')::date;
  _days := extract(day from _to)::int;

  if exists (select 1 from public.payroll_runs
              where entity_id = _entity_id and period = _period and status = 'Published') then
    raise exception 'payroll for % is already published — unpublish it first', _period;
  end if;

  insert into public.payroll_runs (entity_id, period, status, run_by)
  values (_entity_id, _period, 'Draft', auth.uid())
  on conflict (entity_id, period) do update set run_by = excluded.run_by
    where public.payroll_runs.status = 'Draft'
  returning id into _run_id;

  -- The upsert locks the run and rechecks its state after a concurrent publication.
  if _run_id is null then
    raise exception 'payroll for % is already published — unpublish it first', _period;
  end if;

  delete from public.payslips where run_id = _run_id;

  for _emp in
    select e.id, e.entity_id, e.zone_id, e.branch_id, e.department_id
      from public.employees e
     where e.entity_id = _entity_id and e.status = 'Active'
  loop
    select * into _sal
      from public.salary_structures s
     where s.employee_id = _emp.id and s.effective_from <= _to
     order by s.effective_from desc
     limit 1;
    continue when _sal.id is null;

    -- The fraction prices a working day; holidays and weekly offs remain paid.
    select coalesce(sum(case when a.day_type = 'working'
                             then greatest(0, 1 - coalesce(a.day_fraction, 0))
                             else 0 end), 0)
      into _lop
      from public.attendance a
     where a.employee_id = _emp.id and a.work_date between _from and _to;

    _paid       := greatest(_days - _lop, 0);
    _gross_paid := round(_sal.gross * _paid / _days, 2);
    _basic_paid := round((_sal.basic / _days) * _paid, 2);

    _earn       := 0;
    _named_earn := 0;
    _ded        := 0;
    _emp_cost   := 0;

    insert into public.payslips (employee_id, period, gross, deductions, net, status,
                                 run_id, paid_days, lop_days, employer_cost,
                                 entity_id, zone_id, branch_id, department_id)
    values (_emp.id, _period, 0, 0, 0, 'Draft', _run_id, _paid, _lop, 0,
            _emp.entity_id, _emp.zone_id, _emp.branch_id, _emp.department_id)
    on conflict (employee_id, period) do update
      set run_id = excluded.run_id, paid_days = excluded.paid_days,
          lop_days = excluded.lop_days, status = 'Draft'
    returning id into _payslip_id;

    delete from public.payslip_lines where payslip_id = _payslip_id;

    insert into public.payslip_lines (payslip_id, code, name, kind, amount, sort_order)
    values (_payslip_id, 'BASIC', 'Basic', 'earning', _basic_paid, 1);
    _earn := _basic_paid;
    _structure_total := _sal.basic;

    for _sal_comp in select * from app.salary_gross_components(_sal.notes) loop
      -- Round cumulative named earnings so a valid breakdown reconciles to the cent
      -- even in partial/31-day months. The rounding remainder stays in its named line.
      _structure_total := _structure_total + _sal_comp.component_amount;
      if _structure_total > _sal.gross then
        raise exception 'Salary breakdown for employee % exceeds agreed monthly gross. Review Basic and named allowances.', _emp.id
          using errcode = '23514';
      end if;
      _sal_comp_paid := round(_structure_total * _paid / _days, 2) - _earn;
      continue when _sal_comp_paid = 0;

      _named_earn := _named_earn + _sal_comp_paid;
      _earn := _earn + _sal_comp_paid;
      insert into public.payslip_lines (payslip_id, code, name, kind, amount, sort_order)
      values (
        _payslip_id,
        'GROSS_' || _sal_comp.component_order,
        _sal_comp.component_name,
        'earning',
        _sal_comp_paid,
        10 + _sal_comp.component_order
      );
    end loop;

    for _comp in select * from app.components_for(_emp.id) loop
      continue when _comp.min_gross is not null and _sal.gross < _comp.min_gross;
      continue when _comp.max_gross is not null and _sal.gross > _comp.max_gross;

      _base := case _comp.calc_type
                 when 'percent_of_basic' then case when _comp.prorate_on_lop then _basic_paid else _sal.basic end
                 when 'percent_of_gross' then case when _comp.prorate_on_lop then _gross_paid else _sal.gross end
                 else 0
               end;
      if _comp.cap_base is not null and _base > _comp.cap_base then
        _base := _comp.cap_base;
      end if;

      _amt := case _comp.calc_type
                when 'fixed' then
                  case when _comp.prorate_on_lop
                       then round(coalesce(_comp.amount, 0) * _paid / _days, 2)
                       else coalesce(_comp.amount, 0) end
                else round(_base * coalesce(_comp.rate, 0) / 100.0, 2)
              end;
      if _comp.max_amount is not null and _amt > _comp.max_amount then
        _amt := _comp.max_amount;
      end if;
      continue when _amt = 0;

      if _comp.employer_share then
        _emp_cost := _emp_cost + _amt;
        insert into public.payslip_lines (payslip_id, code, name, kind, amount, sort_order)
        values (_payslip_id, _comp.code, _comp.name, 'employer', _amt, 700 + _comp.display_order);
      elsif _comp.kind = 'earning' then
        _named_earn := _named_earn + _amt;
        _earn := _earn + _amt;
        insert into public.payslip_lines (payslip_id, code, name, kind, amount, sort_order)
        values (_payslip_id, _comp.code, _comp.name, 'earning', _amt, 100 + _comp.display_order);
      else
        _ded := _ded + _amt;
        insert into public.payslip_lines (payslip_id, code, name, kind, amount, sort_order)
        values (_payslip_id, _comp.code, _comp.name, 'deduction', _amt, 500 + _comp.display_order);
      end if;
    end loop;

    -- General allowances and employee-specific names both describe the agreed gross.
    -- Keep every configured line; an incompatible breakdown needs payroll review,
    -- not silently dropped allowances or a higher contractual gross. The exception
    -- rolls the entire run back, including any previous draft being recomputed.
    if _earn > _gross_paid then
      raise exception 'Earning components for employee % total %, exceeding payable gross %. Review the salary breakdown and part-of-gross allowances.',
        _emp.id, _earn, _gross_paid using errcode = '23514';
    end if;
    _other := round(_gross_paid - _basic_paid - _named_earn, 2);
    if _other > 0 then
      insert into public.payslip_lines (payslip_id, code, name, kind, amount, sort_order)
      values (_payslip_id, 'OTHER', 'Other allowances', 'earning', _other, 300);
      _earn := _earn + _other;
    end if;

    update public.payslips
       set gross = _earn, deductions = _ded, net = _earn - _ded, employer_cost = _emp_cost
     where id = _payslip_id;

    _count     := _count + 1;
    _sum_gross := _sum_gross + _earn;
    _sum_net   := _sum_net + (_earn - _ded);
  end loop;

  update public.payroll_runs
     set employees = _count, total_gross = _sum_gross, total_net = _sum_net
   where id = _run_id;

  return jsonb_build_object('run_id', _run_id, 'period', _period, 'employees', _count,
                            'total_gross', _sum_gross, 'total_net', _sum_net);
end $$;

grant execute on function public.run_payroll(uuid, text) to authenticated;

-- Publishing is a release of the reviewed statement, not a payment transaction.
create or replace function public.publish_payroll(_run_id uuid)
returns void language plpgsql security definer set search_path = app, public as $$
declare _ent uuid;
begin
  select entity_id into _ent from public.payroll_runs where id = _run_id for update;
  if _ent is null then raise exception 'run not found'; end if;
  if auth.uid() is not null and not app.has_perm('payroll.manage', _ent, null, null, null, null) then
    raise exception 'not authorized to publish payroll for this entity';
  end if;

  update public.payroll_runs
     set status = 'Published', published_at = coalesce(published_at, now())
   where id = _run_id;
  update public.payslips set status = 'Published' where run_id = _run_id and status = 'Draft';
end $$;

-- 0041 wrote Paid during publication; 0095 subsequently hid those historical rows.
-- Repair only rows with a proven published parent. Orphan Paid rows and rows in draft
-- runs have no publication proof and remain hidden under the unchanged strict RLS.
update public.payslips ps
   set status = 'Published'
  from public.payroll_runs r
 where ps.run_id = r.id and r.status = 'Published' and ps.status = 'Paid';

revoke execute on function public.run_payroll(uuid, text), public.publish_payroll(uuid) from public, anon;
grant execute on function public.run_payroll(uuid, text), public.publish_payroll(uuid) to authenticated, service_role;

commit;
