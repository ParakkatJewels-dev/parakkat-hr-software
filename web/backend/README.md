...
└── supabase/
    ...
    │   └── 00xx_*.sql             later additions (audit log, etc.)
...
2. `cp .env.local.example .env.local` and fill `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`.
   (The frontend uses `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` in `frontend/.env.local`.)
3. Run every file in `supabase/migrations/` **in numeric order** in the dashboard SQL Editor.
