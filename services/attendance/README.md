...
| `GET /api/status` | global `device.manage` / `attendance.manage` |
...
The API applies admission limits before expensive work:

| Budget | Limit |
| --- | --- |
| Incoming requests, before JSON parsing and Supabase session verification | 600 per client IP/minute; 1,800 per process/minute |
| Authenticated ordinary reads | 120 per user/minute |
| Authenticated ordinary writes | 30 per user/minute |
| Exports, syncs, backfills, recomputes and mapping suggestions, combined | 20 per user/minute |
| Expensive HTTP operations still running | 4 per process |
| Accepted background operations still running after HTTP 202 | 4 per process |

Excess requests receive HTTP **429** with a `Retry-After` duration in seconds. The header is
exposed to approved browser origins. Closing a browser connection does not free an expensive
operation's slot while its database/device work is still running. Quota maps hold at most 2,000
identities, expire after a minute, and refuse new identities at capacity instead of evicting an
active quota. These are process-local protections; running several workers multiplies capacity.
Use a shared ingress quota if the service is deployed across several processes or machines.

Client IPs come from the actual socket by default. When running behind a reverse proxy, configure
`API_TRUST_PROXY` with only that proxy's IP/CIDR (for example `127.0.0.1/32,::1/128` for a proxy on
the same machine). The default empty value ignores `X-Forwarded-For`; do not trust arbitrary
internet-supplied forwarding headers. Several devices behind one NAT share the IP budget but
keep independent authenticated user budgets.

Health database checks and global service diagnostics are cached in memory for **5 seconds**;
concurrent callers share one read. Every diagnostics request still passes its global permission
check before reading that cache. Session verification similarly shares concurrent requests for
the same token, uses at most 100 pending verifications, and has an 8-second timeout per upstream
request. Verified permissions are cached for at most **60 seconds**, never beyond the token's
expiry, with at most 2,000 entries. A permission change may therefore take up to one minute to
reach this service. Attendance/payroll reports are not response-cached; HTTP responses use
`private, no-store` to keep employee data out of browser/proxy caches.

These controls cover this Express service. The web app's direct Supabase Auth, PostgREST/RPC,
Storage and Realtime traffic, and the optional `invite-user` Edge Function, do not pass through
it. Their project/edge abuse controls must be managed at those platform entrypoints. The separate
developer integration API also has a shared PostgreSQL per-key quota; see
[Developer API](../../web/backend/DEVELOPER_API.md).

`npm run test:roles` uses the standard role grants and actual `get_my_access` payloads generated by
the web backend's disposable migration replay. It starts its own private PostgreSQL cluster and
Express server, checks scoped JSON and XLSX output and cross-branch/zone/company denials, and
captures the employee IDs passed to recompute. Supabase token verification and operational jobs are
fixtures, so the test neither contacts a real identity provider nor runs device synchronization.
Department and self grants deliberately cannot request bulk service exports; their regular scoped
application views remain governed by Supabase RLS. Detailed service diagnostics and device operations
require global authority because their payloads cannot be limited to one company or branch.
